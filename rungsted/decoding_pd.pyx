...
cdef class Viterbi(object):
    ...
    cdef _build_chart(self, int n_tokens):
        ...
        c = ChartBuilder(items, outputs, lattice=True)
        # Terminal nodes
    ...
    cdef double[:, :, ::1] _make_scores(self, Sequence sent, outputs):
        ...
        for j in range(self.n_labels):
            output_scores[0, j, self.START_LABEL] = self.transition.get2d(j, self.START_LABEL)
        ...
        for i in range(sent.examples.size()):
            ...
            for cur_label in range(self.n_labels):
                ...
                for next_label in range(self.n_labels):
                    t_score = self.transition.get2d(next_label, cur_label)
        ...
        for j in range(self.n_labels):
            e_score = self.emission.score(&cur, cur_label, self.feat_map)
            t_score = self.transition.get2d(self.FINAL_LABEL, j)
